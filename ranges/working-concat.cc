...
public:
  ...
  using difference_type =
      std::common_type<ranges::range_difference_t<V1>,
                       ranges::range_difference_t<V2>>::type;
...
static_assert(
    std::forward_iterator<concat_view<ranges::single_view<int>,
                                      ranges::single_view<int>>::iterator>);
static_assert(ranges::view<ranges::single_view<int>>);
static_assert(ranges::view<
              concat_view<ranges::single_view<int>, ranges::single_view<int>>>);
...
int main() {
  ...
  auto chain = concat_view(views::all(v1), views::all(v2));
  for (auto i : chain)
    std::cout << i << " ";
  std::cout << std::endl;

  auto first_two = concat_view(views::single(2), views::single(3));
  ...
  auto primes = concat_view(first_two, others);
